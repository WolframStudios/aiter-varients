...
def resolve_config_dir(
    ...
    to the last candidate so the caller's missing-file assertion names a
    real probe path.

    legacy_dir appends the legacy flat arch-prefixed candidates under
    configs/<legacy_dir>/. GEMM no longer passes it (configs/gemm/ holds no
    GEMM families); it is reserved for the future MOE unification
    (configs/CLAUDE.md section 5).
    """
...
def _get_gemm_config_cached(
    ...
    default named ``DEFAULT.json``); ``backend=None`` tries triton then
    gluon. The legacy flat ``gemm/`` layout is no longer consulted.
    """
    # Input validation
    ...
    # Nested layout <arch>/<backend>/gemm/<d_type>/ only (no arch prefix,
    # default named DEFAULT.json); the shared probe lives in
    # resolve_config_dir().
